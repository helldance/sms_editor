...
public class EditSmsDiagFragment extends DialogFragment {
    ...
    private void modify(ContentValues cv, String after_addr, String orig_addr) {
        ...
        if (!after_addr.equalsIgnoreCase(orig_addr)) {
            ...
            getActivity().getContentResolver().delete(getActivity().getContentResolver().insert(uri, cv_empty2), null, null);
        }

        //getActivity().getContentResolver().delete(getActivity().getContentResolver().insert(uri, cv), null, null);
        ContentValues cv_empty = new ContentValues();
        cv_empty.put("address", sender.getText().toString());
        cv_empty.put("read", 1);
        getActivity().getContentResolver().delete(getActivity().getContentResolver().insert(uri, cv_empty), null, null);
